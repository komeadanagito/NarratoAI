...
| `seed_audio.api_url` | `https://openspeech.bytedance.com/api/v3/tts/unidirectional` |
| `seed_audio.model` | `seed-tts-1.1`，可由服务端配置覆盖 |
| `seed_audio.app_id` | 环境变量 `SEED_AUDIO_APP_ID` 或本地配置 |
| `seed_audio.access_token` | 环境变量 `SEED_AUDIO_ACCESS_TOKEN` 或本地配置 |
| `seed_audio.voice_type` | 默认音色；请求中的 `voice_id` 可覆盖 |

真实密钥不得进入 Git、日志、批次状态或错误响应。当前旧版 `doubaotts_tts()` 使用 `/api/v1/tts`，不能当作 Seed Audio v3 已完成；必须通过独立 Provider 适配并使用 mock 测试请求格式。V3 HTTP 鉴权使用服务端配置的 AppID 与 Access Token，业务 API 不接收供应商凭据。

官方大模型 V3 HTTP 接口默认输出 24 kHz，当前文档只列出 8/16 kHz 作为可选采样率，因此后端不向供应商发送无效的 48 kHz 参数。`voice_prompt` 会尽力映射为音色支持的标准情绪值；任意自然语言语音指令及其可用范围仍取决于供应商协议和所选音色。
