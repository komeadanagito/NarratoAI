...
| 完整视频理解和解说文案 | 新增 `VideoNarrationService` | 将视频直接提交给多模态模型，一次返回最终结构化脚本；不抽帧、不二次调用文本模型 |
...
| 批量调度 | 缺失 | 按请求的 `concurrency` 创建线程调度，不设置固定 worker 上限 |
...
└── services/
    ...
    ├── batch_processor.py          # 批次创建、按请求并发调度、状态聚合
    ├── batch_store.py              # 线程安全的内存状态快照
    ├── deduplication_service.py    # FFmpeg 内容变换
    ├── narration_pipeline.py       # 完整视频理解、TTS、字幕、合成
    ├── documentary/
    │   └── video_narration_service.py # 视频模型直接生成结构化解说脚本
...
| `concurrency` | 否 | 默认 1；必须是大于等于 1 的严格整数，API 和服务端配置均不设置上限 |
...
- 请求中的 `concurrency` 必须是大于等于 1 的严格整数，不能用布尔值或字符串代替。
- 不设置固定的服务端 worker 上限；调度器按照请求值建立并发能力。单个批次实际同时运行的任务数为：

```text
min(请求 concurrency, 批次中的视频任务数)
```

- 调用方必须根据机器的 CPU、内存、磁盘和上游 API 配额合理填写 `concurrency`。
- 视频任务并发数与单个 FFmpeg 的 `-threads` 分开配置；接口不替调用方限制资源使用。
...
当 `narration.enabled=false` 时，不允许调用视频模型或 TTS，直接保留原声并进入内容变换。

当 `narration.enabled=true` 时：

1. 使用 `VideoNarrationService` 将完整视频直接提交给服务端配置的多模态视频模型。
2. 视频模型在一次请求中直接返回最终结构化解说脚本（含真实时间戳、画面描述和口播）；禁止抽帧链路及文本模型二次改写。
3. 根据 `language`、可选 `voice_id` 和 `voice_prompt` 调用 Seed Audio。
4. 生成解说音频和字幕时间轴；解说片段统一使用 `OST=0`，最终成片不保留原视频音轨。
...
| 配置项 | 默认/示例值 |
| --- | --- |
| `llm_profiles.*.base_url` | `https://ark.cn-beijing.volces.com/api/v3` |
| `llm_profiles.*.models.vision` | 支持视频理解的多模态模型，例如 `doubao-seed-2-1-turbo-260628` |
| `llm_profiles.*.api_key_env` | 环境变量 `VOLCANO_ARK_API_KEY` |
| `tts_profiles.*.api_url` | `https://openspeech.bytedance.com/api/v3/tts/create` |
| `tts_profiles.*.model` | `seed-audio-1.0` |
| `tts_profiles.*.api_key_env` | 环境变量 `SEED_AUDIO_API_KEY` |
| `tts_profiles.*.speaker` | 可选；未配置时使用纯文本生成，配置后可由请求中的 `voice_id` 覆盖 |
| `tts_profiles.*.audio_config.sample_rate` | `48000` |

真实密钥不得进入 Git、日志、批次状态或错误响应。Ark 使用 OpenAI-compatible
chat completions；Seed Audio create 接口使用 `X-Api-Key`，业务 API 不接收供应商凭据。
模型切换统一修改 `config/models.json` 的 profile 和 active 指针。

视频模型使用 OpenAI-compatible `video_url` 内容块。本地视频以 Base64 data URL
直传，`llm_video_fps` 默认 `1.0`，单文件上限由 `llm_video_max_bytes` 控制，默认
`52428800`（50 MiB）。超过限制时任务明确失败，禁止回退到抽帧。
生产调用使用 `llm_video_temperature=0.2` 和 `llm_video_max_tokens=4096`，避免继承
通用视觉模型过大的输出上限。可使用 `scripts/debug_narration_request.py` 对完整视频发起
一次零重试真实请求；Seed Audio 使用独立的 `scripts/debug_seed_audio_request.py` 测试。
...
- 实现线程安全 BatchStore。
- 实现按请求 `concurrency` 调度的 BatchProcessor 和状态聚合。
- 接通创建批次与查询批次路由。
- 验证请求并发数、部分成功和队列满行为。
...
- 五个路由的成功状态码和响应结构。
- `BatchCreateRequest` 必填字段、严格类型、正整数并发和未知字段拒绝。
- 所有枚举值与 YAML 一致。
...
- API 接受 concurrency 为 1 和 1000，拒绝 0、布尔值和字符串；调度测试使用与机器资源相称的并发值。
...
6. 多视频任务按请求的 `concurrency` 执行，单个失败不影响其他任务。
...
| 并发线程数 | API 与服务端配置不设置上限；调用方负责根据机器资源选择正整数 |
| Seed Audio 时间戳 | 需确认是否返回字/句级时间戳；否则采用分段时长或 ASR 回标 |
| Seed Audio 音色 | 需确认预置音色与声音克隆的授权、保存和删除规则 |
| Seed Audio 采样率/语音指令 | V3 HTTP 当前使用 24 kHz；自由语音指令仅做标准情绪映射，不声明原生 48 kHz 或任意指令支持 |
